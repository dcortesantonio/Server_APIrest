...
func InfoDomain(w http.ResponseWriter, r *http.Request) {
	...
	infoServer := ModelsAPI.Server{}
	serversAux := []ModelsAPI.ServerItem{}
	MinGrade := 0
	...
	for _, endpoint := range host.Endpoints {
		...
		aux := ModelsAPI.ServerItem{}
		aux.Address = endpoint.IpAddress
		aux.SSL_Grade = endpoint.SSL_Grade
	...
	infoServer.Min_SSL_Grade = grades[MinGrade]
